...
export const addData=(mail)=>{
  return async(dispatch)=>{
    ...
    try{
      const response=await axios.post(`https://ecommerce-121a5-default-rtdb.firebaseio.com/${senderMail}.json`,{mail});
      await axios.post(`https://ecommerce-121a5-default-rtdb.firebaseio.com/${receiverMail}.json`,{mail});
      console.log('success')
...
export const getMail=()=>{
 ...
 const newEmail= email.replace('@',"").replace('.','');
  try{
    const response=await axios.get(`https://ecommerce-121a5-default-rtdb.firebaseio.com/${newEmail}.json`)
    let loadedInboxData=[];
    let loadedSentBox=[]
...
export const deleteMail=(id)=>{
  return async(dispatch)=>{
    ...
    try{
      await axios.delete(`https://ecommerce-121a5-default-rtdb.firebaseio.com/${email}/${id}.json`);
